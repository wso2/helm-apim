...
- Before running the Helm install commands, edit `default_acp_values.yaml`, `default_tm_values.yaml`, and `default_gw_values.yaml` and set `wso2.apim.configurations.encryption.key` to the same shared value in all files so that all API-M nodes and components use an identical key.
