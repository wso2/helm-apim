...
- Before running the Helm install commands, edit `default_values.yaml`, `default_km_values.yaml`, and `default_gw_values.yaml` and set `wso2.apim.configurations.encryption.key` to the same shared value in all files so that all API-M nodes and components use an identical key.
...
- Run the following commands to deploy the Helm charts:
  ...
  1. Deploy All-in-one
      ...
      ```bash
      helm install apim wso2/wso2am-all-in-one --version 4.7.0-1 -f default_values.yaml -n <namespace>
      ```

  2. Deploy KM

      ```bash
      helm install apim-km wso2/wso2am-acp --version 4.7.0-1 -f default_km_values.yaml -n <namespace>
      ```

  3. Deploy GW

      ```bash
      helm install apim-gw wso2/wso2am-gw --version 4.7.0-1 -f default_gw_values.yaml -n <namespace>
      ```
