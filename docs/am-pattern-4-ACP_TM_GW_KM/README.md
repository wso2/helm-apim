...
- Before running the Helm install commands, edit `default_acp_values.yaml`, `default_km_values.yaml`, `default_tm_values.yaml`, and `default_gw_values.yaml` and set `wso2.apim.configurations.encryption.key` to the same shared value in all files so that all API-M nodes and components use an identical key.
...
1. Deploy ACP
    ...
    ```bash
    helm install apim-acp wso2/wso2am-acp --version 4.7.0-1 -f default_acp_values.yaml -n apim
    ```

2. Deploy KM

    ```bash
    helm install apim-km wso2/wso2am-km --version 4.7.0-1 -f default_km_values.yaml -n apim
    ```

3. Deploy TM

    ```bash
    helm install apim-tm wso2/wso2am-tm --version 4.7.0-1 -f default_tm_values.yaml -n apim
    ```

4. Deploy GW

    ```bash
    helm install apim-gw wso2/wso2-gw --version 4.7.0-1 -f default_gw_values.yaml -n apim
    ```
